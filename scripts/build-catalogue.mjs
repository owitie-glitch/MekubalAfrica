...
/**
 * Shop-floor photographs. Too busy to sell a single piece, but they show the
 * Westlands stall as it actually is, which no styled product shot conveys.
 * Rendered as an editorial strip rather than as products.
 *
 * Left unnormalised — the dim interior light is the point.
 */
const LOOKBOOK = [
...
// These were shot over several sessions on different surfaces, and measured
// mean luminance ranged from 54 to 170 — a threefold spread that reads as a
// jumble however the grid is laid out. Each photograph is nudged toward a
// common exposure so the catalogue looks like one set.
const TARGET_LUMA = 150;
// Clamped: an unbounded correction would blow the highlights out of the
// darkest frames rather than rescuing them.
const MIN_GAIN = 0.92;
const MAX_GAIN = 1.5;

async function meanLuma(pipeline) {
  const { channels } = await pipeline.stats();
  const [r, g, b] = channels;
  return r.mean * 0.299 + g.mean * 0.587 + b.mean * 0.114;
}

async function optimise(fileName, slug, index, { normalise = true } = {}) {
  ...
  let pipeline = base;
  if (normalise) {
    const luma = await meanLuma(base.clone());
    const gain = Math.min(MAX_GAIN, Math.max(MIN_GAIN, TARGET_LUMA / luma));
    pipeline = base.clone().modulate({ brightness: gain });
  }

  const out = `${slug}-${index + 1}.webp`;
  await pipeline.webp({ quality: 72 }).toFile(path.join(OUT, out));
  return `/uploads/${out}`;
}
...
async function main() {
  ...
  const lookbook = [];
  for (const [i, shot] of LOOKBOOK.entries()) {
    const url = await optimise(shot.file, "lookbook", i, { normalise: false });
    if (url) lookbook.push({ url, caption: shot.caption });
  }
  // Written to source rather than the database: these are fixed editorial
  // assets, not stock the shop edits.
  await writeFile(
    path.join(process.cwd(), "src", "lib", "lookbook.ts"),
    `// Generated by scripts/build-catalogue.mjs — do not edit by hand.\n` +
      `export const lookbook = ${JSON.stringify(lookbook, null, 2)} as const;\n`,
  );
  console.log(`  ${lookbook.length} shots`);
